      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-2xl font-black tracking-tight text-foreground flex items-center gap-2">
            ...
            Gestión de <span className="text-orange-600">Clases</span>
          </h1>
          <p className="text-foreground/50 text-xs font-medium">Administra las clases de tu catálogo de productos.</p>
        </div>
      </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  placeholder="Ej. Accesorios Eléctricos"
                  required 
                  autoComplete="off"
                />
              </div>
            ...
            <div className="divide-y divide-border max-h-[600px] overflow-y-auto">
              {loading ? (
                ...
                filteredClasses.map((cls) => (
                  <div key={cls.id} className="grid grid-cols-12 gap-4 p-4 items-center hover:bg-foreground/[0.01] transition-colors group">
                    <div className="col-span-8 font-bold text-sm text-foreground uppercase">
                      {cls.name}
                    </div>
                    <div className="col-span-3 flex justify-center">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${cls.isEnabled ? 'bg-green-500/10 text-green-600 border border-green-500/20' : 'bg-red-500/10 text-red-600 border border-red-500/20'}`}>
                ...
                <div className="p-12 text-center text-foreground/40 font-semibold text-sm">
                  {searchTerm ? "No se encontraron clases que coincidan con la búsqueda." : "Aún no hay clases creadas."}
                </div>
              )}
            </div>
          </div>
